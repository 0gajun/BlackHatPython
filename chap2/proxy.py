...
def request_handler(request):
    if b"admin" in request:
        request = request.replace(b"admin", b"vagrant")
    return request
...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    while True:
        ...
        if not len(local_buffer) or not len(remote_buffer):
            client_socket.close()
            remote_socket.close()
            print("[*] No more data. Closing conenections")
...
def hexdump(src, length=16):
    ...
    src = src.decode() if type(src) == bytes else src

    for i in range(0, len(src), length):
        s = src[i:i + length]
        hexa = b' '.join([b"%0*X" % (digits, ord(x)) for x in s])
        text = b''.join([x.encode()
                         if 0x20 <= ord(x) < 0x7F else b'.' for x in s])
